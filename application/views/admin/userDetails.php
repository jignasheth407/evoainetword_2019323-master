												...
												<div class="row">
													...
													<div class="form-group col-md-3">
														<div class="input text">
															...
															<p><input type="checkbox" <?php echo ($user_result->super_role == 1)?'checked':'';?> id="super_role" onChange="setSuperVersion(this)"> &nbsp; All access enable</p>
														</div>
													</div>
												</div>
											</div>
										...
										<div class="tab-pane" id="tab_2">
											<div class="box-body">
																...
																{ 
																	?>
																		<tr>
																			...
																			<td>
																				<?php
																					$bonusData_result = $this->db->select_sum('bonus_value')->get_where('user_coin_transaction', array('user_id'=>$res->id))->result();
																					$total_bonus = $total_bonus + $bonusData_result[0]->bonus_value;
																					echo $bonusData_result[0]->bonus_value;
																				?>																				
																			</td>																																								
																			<td>
																				<?php
																					$paidData_result = $this->db->select_sum('paid_value')->order_by('id', 'desc')->get_where('user_coin_transaction', array('user_id'=>$res->id))->result();
																					//$paid_bonus = $paid_bonus + $paidData_result[0]->paid_value;
																					$paid_bonus = $paidData_result[0]->paid_value;
																					echo $paidData_result[0]->paid_value;
																				?>	
																			</td>
																			<td>
												...
												<table class="table table-bordered table-striped">
													<thead>
														<tr>
															<th width="20%">Remain bonus</th>
															<th width="20%">
																<?php
																	$cal_paid_bonus = $total_bonus - $paid_bonus;
																	echo $cal_paid_bonus;
																?>
															</th>
															<th width="20%">Paid bonus</th>
															<th width="20%">
																<?php
																	echo $paid_bonus;
																?>
															</th>
															<th width="20%" class="hidden"><span class="btn btn-success">&nbsp;&nbsp;Pay now</span></th>
														</tr>
													</thead>
												</table>
											</div>
	...
	<script>
		...
		/* setSuperVersion */
		function setSuperVersion(checkboxElem)
		{
			var userSuper_val = '';
			var user_id = '<?= $user_result->id ?>';
			if (checkboxElem.checked) {
			...
			var dataString = 'userSuperVersion='+userSuper_val+'&user_id='+user_id;
			$.ajax({
				url : '/admin/users/setSuperVersion',
				type: 'POST',
				data: dataString,
				cache: false,
				success: function(response)
